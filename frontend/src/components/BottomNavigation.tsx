      ...
      <PopoverTrigger
        render={
          <button
            ...
            onMouseEnter={() => setHovered(true)}
            onMouseLeave={() => setHovered(false)}
            className={`h-8 inline-flex items-center justify-center rounded-lg text-sm font-medium transition-all duration-300 ease-out overflow-hidden whitespace-nowrap bg-background/95 backdrop-blur text-foreground border border-border hover:bg-muted dark:bg-background/95 dark:border-border dark:hover:bg-muted ${isActive || open ? "ring-2 ring-ring/30" : ""}`}
            style={{
              width: hovered ? "auto" : "2rem",
              minWidth: "2rem",
              paddingLeft: hovered ? "0.75rem" : "0.5rem",
